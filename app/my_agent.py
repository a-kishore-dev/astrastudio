...
from mcp_utils import build_agent_mcps
...
class MyAgent:
    ...
    def get_crewai_agent(self) -> Agent:
        ...
        # MCP integration (CrewAI 1.5+ uses `mcps`, not `mcp_servers`)
        mcps = []
        if 'mcps' in ss and self.mcp_ids:
            selected_entries = [
                m for m in ss.mcps
                if m["id"] in self.mcp_ids and m.get("enabled", True)
            ]
            mcps = build_agent_mcps(selected_entries)
    ...
    def delete(self):
        # Filter the session state list
        ss.agents = [agent for agent in ss.agents if agent.id != self.id]
        # Call DB with user_id
        db_utils.delete_agent(self.id, ss.user_id)
                ...
                with st.form(key=form_key):
                    ...
                    if 'knowledge_sources' in ss and len(ss.knowledge_sources) > 0:
                        ...
                        if len(valid_knowledge_sources) != len(self.knowledge_source_ids):
                            ...
                            db_utils.save_agent(self, ss.user_id)
    ...
    def set_editable(self, edit):
        self.edit = edit
        # Pass the user_id from session state
        db_utils.save_agent(self, ss.user_id)
        if not edit:
            st.rerun()