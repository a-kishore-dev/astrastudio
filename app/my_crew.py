...
def ensure_process_enum(process_val):
    from crewai import Process
    if isinstance(process_val, Process):
        return process_val
    
    # Clean the string (handles "Process.sequential" or "sequential")
    proc_str = str(process_val).lower()
    if 'hierarchical' in proc_str:
...
class MyCrew:
    ...
    def get_crewai_crew(self, *args, **kwargs) -> Crew:
        ...
        if 'knowledge_sources' in ss and self.knowledge_source_ids:
            ...
            if len(valid_knowledge_source_ids) != len(self.knowledge_source_ids):
                ...
                db_utils.save_crew(self, ss.user_id)

        process_enum = ensure_process_enum(self.process)
    ...
    def delete(self):
        ...
        db_utils.delete_crew(self.id, ss.user_id)
                ...
                if st.button("Delete crew + selected items", type="primary"):
                    ...
                    if selected_task_ids:
                        ...
                        for tid in selected_task_ids:
                            db_utils.delete_task(tid, ss.user_id)
                    if selected_agent_ids:
                        ...
                        for aid in selected_agent_ids:
                            db_utils.delete_agent(aid, ss.user_id)
                    self.delete()
                    self.clear_delete_modal()
                    st.rerun()
