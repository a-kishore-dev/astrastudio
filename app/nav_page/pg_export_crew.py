...
"""
    ...
    def export_crew_to_json(self, crew):
        ...
        for agent in crew.agents:
            agent_data = {
                ...
                'mcp_ids': agent.mcp_ids if hasattr(agent, 'mcp_ids') else []
            }
            crew_data['agents'].append(agent_data)
            tool_ids.update(agent_data['tool_ids'])
            mcp_ids.update(agent_data['mcp_ids'])
        ...
        for m_id in mcp_ids:
            mcp_entry = next((m for m in ss.mcps if m["id"] == m_id), None)
            if mcp_entry:
                crew_data['mcp_configs'].append(mcp_entry)

        return json.dumps(crew_data, indent=2)
    
    def import_crew_from_json(self, crew_data):
        if 'mcp_configs' in crew_data:
            for mcp in crew_data['mcp_configs']:
                # Check if MCP already exists by name/endpoint to prevent duplicates
                if not any(ex['name'] == mcp['name'] for ex in ss.mcps):
                    ss.mcps.append(mcp)
    ...
    def draw(self):
        ...
        if st.button("Export everything to json"):
            ...
            db_utils.export_to_json(file_path, ss.user_id)
        ...
        if uploaded_file is not None:
            ...
            if isinstance(json_data, list):  # Full database export
                ...
                db_utils.import_from_json("uploaded_file.json", ss.user_id)
